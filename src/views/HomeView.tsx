...
function MenuRow({ icon, label, hasArrow, danger, onClick }: MenuRowProps) {
  ...
  return (
    ...
    >
      <span style={{ color: col, display: 'flex', alignItems: 'center', flexShrink: 0 }}>{icon}</span>
      <span style={{ fontSize: 13, color: col, flex: 1 }}>{label}</span>
      {hasArrow && <span style={{ color: col, fontSize: 12, opacity: 0.7 }}>›</span>}
    </div>
  )
}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Folder size={17} strokeWidth={1.8} color={isDropOver ? C_PEAK : active ? C_TEXT : C_BASE} />
        <span style={{ fontSize: 13.5, fontWeight: 500, color: C_TEXT, whiteSpace: 'nowrap' as const }}>
          {folder.name}
        </span>
      </div>
...
function ChatCard({ chat, index, isDragging, onNavigate, onMenuOpen, onDragStart, onDragEnd }: ChatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ position: 'absolute', top: 9, right: 9, opacity: hov ? 1 : 0, transition: 'opacity 0.14s', zIndex: 2 }}
        onClick={onMenuOpen}
      >
        <div
          style={{ padding: '3px 4px', borderRadius: 5, color: C_MUTED, display: 'flex', alignItems: 'center', cursor: 'pointer' }}
          onMouseEnter={e => { ;(e.currentTarget as HTMLDivElement).style.background = 'rgba(58,138,170,0.18)'; ;(e.currentTarget as HTMLDivElement).style.color = C_TEXT }}
          onMouseLeave={e => { ;(e.currentTarget as HTMLDivElement).style.background = 'transparent'; ;(e.currentTarget as HTMLDivElement).style.color = C_MUTED }}
        >
          <MoreVertical size={14} strokeWidth={2} />
        </div>
      </div>

      <div style={{ fontSize: 13.5, fontWeight: 500, color: C_TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, paddingRight: 22 }}>
...
interface MenuState { chatId: string; x: number; y: number }
...
export default function HomeView({ onNewChat, onSelectChat }: Props) {
  ...
  const reload = useCallback(async () => {
    try {
      const [f, c] = await Promise.all([listFolders(), listChats()])
      setFolders(f)
      setChats(c)
    } catch (err) {
      console.error('[aria] failed to load home data:', err)
    }
  }, [])
  ...
  const handleMoveChat = async (chatId: string, folderId: string) => {
    try {
      ...
      console.error('[aria] moveChat failed:', err)
    }
    setMenu(null)
    setShowSub(false)
  }
  ...
  const handleDeleteChat = async (chatId: string) => {
    try {
      ...
      console.error('[aria] deleteChat failed:', err)
    }
    setMenu(null)
  }
  ...
  const handleFolderDrop = async (e: React.DragEvent, folderId: string) => {
    ...
    if (chatId) {
      try {
        ...
        console.error('[aria] drag-drop moveChat failed:', err)
      }
    }
    setOverFolder(null)
    setDraggingId(null)
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{
        ...
        {/* Folders */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, delay: 0.08 }} style={{ flexShrink: 0 }}>
        ...
        {/* Recent header */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, delay: 0.20 }} style={{ flexShrink: 0, marginBottom: 14 }}>
          <SectionLabel>Recent</SectionLabel>
        </motion.div>

        {/* Scrollable cards — ONLY scrollable area */}
        <div style={{
          flex: 1, minHeight: 0,
          ...
          {chats.length === 0 ? (
            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C_MUTED, fontSize: 13, fontStyle: 'italic' }}>
              No conversations yet.
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 11, paddingBottom: 20 }}>
              {chats.map((chat, idx) => (
                <ChatCard
                  key={chat.id}
                  chat={chat}
                  ...
                  onNavigate={() => onSelectChat(chat.id, chat)}
      ...
      <div style={{
        ...
        <motion.div
          ...
          style={{ fontSize: 52, fontWeight: 250, color: C_TEXT, letterSpacing: '0.04em', marginTop: 16, lineHeight: 1, userSelect: 'none' }}
        >
          ARIA
        </motion.div>

        <motion.div
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.45, delay: 0.13 }}
          style={{ fontSize: 17, fontWeight: 500, color: C_TEXT, letterSpacing: '0.09em', marginTop: 7, userSelect: 'none', textAlign: 'center' }}
        >
          Advanced Researching &amp; Intelligence Assistant
        </motion.div>

        <motion.div
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.45, delay: 0.19 }}
          style={{ fontSize: 13, color: C_MUTED, fontStyle: 'italic', marginTop: 13, letterSpacing: '0.01em', userSelect: 'none' }}
        >
          A presence, not a service.
        </motion.div>

        <motion.button
          initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, delay: 0.26 }}
          onClick={onNewChat}
          ...
          style={{
            ...
            padding: '10px 26px', borderRadius: 32,
            ...
            fontSize: 13.5, fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '0.02em',
        ...
        {menu && (
          <motion.div
            ...
            style={{
              ...
              borderRadius: 10, padding: '5px 0', minWidth: 186,
          ...
          >
                ...
                {showSub && (
                  <motion.div
                    ...
                    style={{
                      ...
                      backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)',
                      border: '1px solid rgba(58,138,170,0.22)',
                      borderRadius: 10, padding: '5px 0', minWidth: 148,
                      boxShadow: '0 10px 36px rgba(0,0,0,0.55)', zIndex: 1001,
                    }}
                  >
                    {folders.map(f => (
                      <MenuRow
                        ...
                        label={f.name}
                        onClick={e => { e.stopPropagation(); void handleMoveChat(menu.chatId, f.id) }}
                      />
                    ))}
                  </motion.div>
                )}
            ...
            <MenuRow
              ...
              label="Delete chat"
              danger
              onClick={e => { e.stopPropagation(); void handleDeleteChat(menu.chatId) }}
            />
          </motion.div>
        )}
      </AnimatePresence>
